...
type node struct {
	//флаг для статичного пути, по дефолту false
	staticpath   bool
	staticprefix string
	staticreal   string
	...
	// список потомков для проверки
	staticIndices []byte
	staticChild   []*node

	// проверка потомков по маске
	wildcardChild *node

	// если не найдено берем все
	catchAllChild *node

	//добавлять слеш в оконцовке
	addSlash bool
...
func (n *node) addPath(path string, wildcards []string, inStaticToken bool, static bool, staticprefix, staticrealpath string) *node {
	//проверка пути на длину
	leaf := len(path) == 0

	//если длина больше 0 то leaf  == true
	if leaf {
		if wildcards != nil {
	...
	if c == '*' && !inStaticToken {
		...
		if static {
			n.staticpath = true
			n.staticprefix = staticprefix
			n.staticreal = staticrealpath
		}
		return n.wildcardChild.addPath(remainingPath, wildcards, false, static, staticprefix, staticrealpath)
		...
		for i, index := range n.staticIndices {
			if c == index {
				...
				if static {
					child.staticpath = true
					child.staticprefix = staticprefix
					child.staticreal = staticrealpath
				}
				return child.addPath(path[prefixSplit:], wildcards, inStaticToken, static, staticprefix, staticrealpath)
			}
		}
		...
		return child.addPath(remainingPath, wildcards, inStaticToken, static, staticrealpath, staticrealpath)
	}
}
...
func (n *node) search(method, path string) (found *node, handler HandlerFunc, params []string) {
	//fmt.Printf("Searching for %s in %s", path, n.dumpTree("", ""))
	//if test != nil {
